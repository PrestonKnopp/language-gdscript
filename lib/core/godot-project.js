...
class GodotProject {
	...
	indexLocalScope(node) {
		...
		this._former.formLocalScopesFromNode(node).forEach((scope) => {
			...
			// iter in reverse order as the symbols are added depth first
			// and we need to solve the types of the top symbols first
			for (let i = symbols.length - 1; i >= 0; i--){
				symbol = symbols[i]
				if (!symbol.info['__node__'])
					return
				if (!symbol.info['type']) {
					const typeSym = this._typeSolver.solve(symbol.info['__node__'])
					if (typeSym)
						symbol.info['type'] = typeSym.name
				}
			}
		})
	}
	...
	indexTree(tree, withName='') {
		...
		const members = Object.values(treeSymbol.info['members'])
		const constants = Object.values(treeSymbol.info['constants'])
		for (const symbols of [members, constants]) {
