...
class GodotProject {
	...
	getSymbolsForAttribute(node, toNode, withPrefix) {
		...
		for (let i = 0, len = node.childCount; i <= len; i++) {
			...
			if (i === 0 && child.type === 'identifier' && child.text === 'self') {
				result = this.getSelfClassSymbolFromNode(node)
				continue
			}
			...
			if ((child.type === 'attribute_subscript') ||
				...
				// HACK: hopefully this resolves when you have `call().|` not getting suggestions
				// because call identifier is not in current scope
				if (child.type === 'call') {
					let tempResult = result.lookup(lookupText)
					if (tempResult) {
						result = this.resolveSymbolType(tempResult)
						continue
					} else {
						result = this.getSelfClassSymbolFromNode(node)
					}
