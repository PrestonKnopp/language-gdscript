...
class Index {
	...
	_lookup(term, useFind=false, fromScope=null) {
		...
		for (const scopeContainer in this._scopeMeta) {
			for (const i in this._scopeMeta[scopeContainer]) {
				const scope = this._scopeMeta[scopeContainer][i]
