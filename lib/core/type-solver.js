...
module.exports = class TypeSolver {
	...
	solveAttributeNode(node, upToNode=null) {
		...
		switch (child.type) {
			case 'identifier':
				lookupResultSymbol = this.solveIdentifierNode(child)
				break
			case 'call':
			case 'base_call':
				lookupResultSymbol = this.solveCallNode(child)
				break
			case 'subscript':
				lookupResultSymbol = this.solveSubscriptNode(child)
				break
	...
	solveSubscriptNode(node) {
		return this.solveCallNode(node) // Same logic
	}

	solveCallNode(node) {
		const ident = treeHelpers.getFirstChildOfNodeWithType(node, 'identifier')
		return this.solveIdentifierNode(ident)
	}
