			...
			if (child === toNode || child.range.containsRange(toNode.range)) {
				if (toNode.type === '.') {
					if (result && result.type === 'class') {
						let results = []
						function procResults(symbolCategory) {
							for (const name in symbolCategory) {
								const sym = symbolCategory[name]
								if (sym.info)
									results.push(sym)
							}
						}
						procResults(result.info['constants'])
						procResults(result.info['methods'])
						procResults(result.info['members'])
						procResults(result.info['classes'])
						result = results
					}
					break
				}
