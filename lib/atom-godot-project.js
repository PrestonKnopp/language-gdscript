...
class AtomParserInterface extends ParserInterface {
	...
	getTreeWithBuffer(buffer) {
		if (buffer.getLanguageMode() && buffer.getLanguageMode().tree) {
			return buffer.getLanguageMode().tree
		}
		...
		// Parsing the buffer is async. mode.tree can be null if tree-sitter hasn't parsed
		// the buffer yet. How do you wait for the buffer to be parsed?
		return mode.tree
	}
...
module.exports = class AtomGodotProject extends GodotProject {
	...
	constructor(resourcePath) {
		...
		this._typeSolver.setIndex(this.index)
		this._typeSolver.setSupplier(this._symbolSupplier)
		this._typeSolver.setFormer(this._former)

		this._symbolSupplier.setIndex(this.index)
	}
	...
	getSymbols(editor, position, prefix, path) {
		...
		if (node.parent && (node.parent.type === 'argument_list' &&
		                   (node.type === 'identifier' || node.type === '('))) {
			result = this.getSymbolsForIdentifier(node)
		} else if (attribute) {
			result = this.getSymbolsForAttribute(attribute, node)
		} else {
			switch (node.type) {
				case 'identifier':
					result = this.getSymbolsForIdentifier(node)
				...
				case 'type':
					result = this._symbolSupplier.getAllClassSymbols()
					break
				case 'name':
					...
					if (!classDefinition) {
						...
						const classSymbol = this._symbolSupplier.getParentClassSymbolOfClassSymbol(this.index.lookup(path))
						result = this._symbolSupplier.getAllMethodSymbolsOfClassSymbolAncestry(classSymbol)
					}
					break
				default: ;
			}
